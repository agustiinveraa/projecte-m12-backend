...
export class UserRepository {
  static async create({ dni, nickname, password, name, surname, birthdate }) {
    ...
    const [existingUser] = await CONNECTION.promise().query('SELECT * FROM user WHERE nickname = ?', [nickname]);
...
class Validation { // TODO: validaciones completas de los campos
  static dni(dni) {
  ...
  static birthdate (birthdate) {
    if (typeof birthdate !== 'string') throw new Error('birthdate must be a string')
    if (!/^\d{4}-\d{2}-\d{2}$/.test(birthdate)) throw new Error('birthdate must be in format YYYY-MM-DD')
  }
}